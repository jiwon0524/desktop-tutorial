...
import { Bell, BookOpen, Brain, CheckCircle2, Download, FileText, Timer, Upload } from "lucide-react";
import { initialWrongNotes, subjects, tabs } from "./data";
...
function Coach({ current }: { current: Subject }) {
  return (
    <aside className="panel coach">
      ...
      <h3>현재 페이지 핵심</h3>
      <p>
        {current.concepts[0]}와 {current.concepts[1]}를 연결해서 봐야 합니다.
      </p>
      <div className="coach-actions">
        {["설명", "요약", "문제 만들기", "암기카드", "쉽게 설명"].map((action) => (
    ...
    <section className="dashboard-grid">
      <article className="panel wide">
        <div className="panel-title">
          <div>
            <span className="eyebrow">Today Loop</span>
      ...
      <article className="panel">
        ...
        <ul className="task-list">
          <li><input type="checkbox" defaultChecked /> {current.concepts[0]} 10분 복습</li>
      ...
      <article className="panel">
        <span className="eyebrow">약점 분석</span>
        <h3>{current.weak}</h3>
      ...
      <article className="panel">
        ...
        <div className="button-row"><button className="primary">시작</button><button>50분</button></div>
      </article>
    </section>
  );
}
    ...
    <section className="room-grid">
      <article className="panel wide">
        ...
        <div className="upload-grid">
          <div className="upload-box"><Upload size={22} /><strong>내 파일 업로드</strong><p>PDF, 이미지, 필기자료, 강의자료, 시험지, 기출문제</p><input type="file" /></div>
          <div className="upload-box"><BookOpen size={22} /><strong>무료/오픈 교재</strong><p>OpenStax, 공개 강의자료, 오픈 텍스트북 연동 설계</p><div className="filter-row"><select><option>{current.title}</option></select><select><option>중급</option><option>초급</option></select><select><option>한국어</option><option>English</option></select></div></div>
          <div className="upload-box"><FileText size={22} /><strong>텍스트 직접 입력</strong><textarea placeholder="개념, 문제, 강의 내용을 붙여넣으세요." /></div>
        </div>
      </article>
  ...
  return (
    <section className="room-grid">
      <article className="panel wide">
        ...
        <div className="rule-box">원본 정답 개념: <strong>{q.concept}</strong><br />문제 표현, 보기, 상황, 예시만 변경. 정답 개념은 절대 변경하지 않음.</div>
        <div className="question-card">
          <span>{q.type} · {q.difficulty}</span>
          <h3>{q.prompt}</h3>
          <ol>{q.choices.map((choice) => <li key={choice}>{choice}</li>)}</ol>
        ...
        <div className="question-card">
          ...
          <div className="choice-list">
            {q.choices.map((choice, index) => (
              <button className={selectedAnswer === index ? "picked" : ""} onClick={() => { setSelectedAnswer(index); setSolved(false); }} key={choice}><span>{index + 1}</span>{choice}</button>
            ))}
          </div>
    ...
    <section className="dashboard-grid">
      ...
      {notes.map((note, index) => (
        <article className="wrong-card" key={`${note.subject}-${note.concept}-${index}`}>
          ...
          <div className="button-row"><button>다시 풀기</button><button className="primary">비슷한 변형 문제 생성</button></div>
        </article>
      ))}
    </section>
  );
...
function Alerts() {
  return (
    <section className="dashboard-grid">
      <article className="panel wide"><span className="eyebrow">알림 학습 목업</span><h2>웹 푸시, 이메일, 추후 카카오 알림톡 확장</h2><div className="notification-list">{["오늘의 개념 3개", "OX 문제 3개", "어제 틀린 문제 복습", "시험 D-day 요약", "과목별 암기 카드", "길가면서 푸는 미니 문제"].map((item) => <label key={item}><input type="checkbox" defaultChecked /> {item}</label>)}</div></article>
      <article className="panel"><span className="eyebrow">로그인 설계</span><h3>카카오 · 이메일 · 게스트</h3><p>1차 MVP는 게스트 체험 흐름으로 구성하고, 계정별 저장소 구조를 분리합니다.</p></article>
    </section>
  );
}

function DownloadPage() {
  const device = /iPhone|iPad/i.test(navigator.userAgent) ? "iOS/iPadOS" : /Android/i.test(navigator.userAgent) ? "Android" : /Mac/i.test(navigator.userAgent) ? "macOS" : /Windows/i.test(navigator.userAgent) ? "Windows" : "Web";
  return (
    <section className="dashboard-grid">
      <article className="panel wide"><span className="eyebrow">멀티 플랫폼 설치</span><h2>1차는 웹앱 + PWA, 이후 데스크톱과 모바일 앱으로 확장</h2><div className="download-grid">{["웹에서 바로 시작", "Windows 다운로드", "Mac 다운로드", "Android 설치", "iPhone/iPad 설치"].map((item, index) => <button className={index === 0 ? "primary" : ""} key={item}><Download size={16} />{item}</button>)}</div></article>
      <article className="panel"><span className="eyebrow">기기 감지</span><h3>{device} 환경</h3><p>PWA 설치 프롬프트와 service worker 준비 구조가 포함되어 있습니다.</p></article>
    </section>
  );
}
  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        <div className="brand"><img src="/assets/icon.svg" alt="" /><div><strong>StudyPilot AI</strong><span>공부하면 점수가 오르는 시스템</span></div></div>
      ...
      <main>
        <aside className="sidebar">
          <div className="create-card">
            ...
            <input value={subjectInput} onChange={(event) => setSubjectInput(event.target.value)} placeholder="예: 정보처리기사, 간호학" />
          ...
          <div className="subject-list">
            {allSubjects.map((item) => (
              <button className={`subject-card ${item.id === selectedSubjectId ? "active" : ""}`} onClick={() => setSelectedSubjectId(item.id)} key={item.id}>
                <div className="subject-head"><span className={`subject-dot ${item.color}`} /><span>{item.field}</span></div>
        ...
        <section className="content">
          <div className="hero">
            ...
            <div className="hero-stats"><div><CheckCircle2 size={22} /><strong>{current.progress}%</strong><span>진도율</span></div><div><Timer size={22} /><strong>{current.readiness}%</strong><span>시험 준비도</span></div></div>
          </div>
